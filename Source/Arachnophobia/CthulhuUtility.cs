...
namespace Arachnophobia
{
    ...
    static public class CthulhuUtility
    {
        ...
        public static bool TryGetUnreservedPewSpot(Thing pew, Pawn claimer, out IntVec3 loc)
        {
            ...
            if (claimer.CanReserve(pew.PositionHeld)) { loc = pew.Position; return true; }

            if (currentDirection == Rot4.North ||
                currentDirection == Rot4.South)
            {
                if (claimer.CanReserve(CellWest)) { loc = CellWest; return true; }
                if (claimer.CanReserve(CellEast)) { loc = CellEast; return true; }
            }
            if (currentDirection == Rot4.East ||
                currentDirection == Rot4.West)
            {
                if (claimer.CanReserve(CellNorth)) { loc = CellNorth; return true; }
                if (claimer.CanReserve(CellSouth)) { loc = CellSouth; return true; }
            }
        ...
        public static void TemporaryGoodwill(Faction faction, bool reset = false)
        {
            ...
            if (!reset)
            {
                ...
                faction.RelationWith(playerFaction, false).kind = FactionRelationKind.Neutral;
            }
            else
            {
                faction.RelationWith(playerFaction, false).goodwill = 0;
                faction.RelationWith(playerFaction, false).kind = FactionRelationKind.Hostile;
            }
        }
